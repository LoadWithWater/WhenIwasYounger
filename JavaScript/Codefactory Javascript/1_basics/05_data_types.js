 ...
 * 한개의 오브젝트 타입이있다.
 * 
 * Primitivie Type : JS의 타입을 구성하는 가장 작은 단위
 * 1) Number (숫자)
 * 2) String (문자열)
 * 3) Boolean (불리언)
 * 4) undefined (언디파인드)
 ...
 * null 타입
 * 
 * undefined의 마찬가지로 값이 없다는 뜻이나
 * JS에서는 개발자가 명시적으로 없는 값으로 초기과할 때 사용됨
 */
let init = null;
console.log(init);
...
/**
 * static typeing -> 변수를 선언할 때 어떤 타입의 변수를 선언할지 명시를 한다.
 *                   C 등
 * dynamic typeing -> 변수의 타입을 명시적으로 선언하지 않고 갑에의해 타입을 "추론"한다.
 *                    JS -> dynamic typing
 */