...
<br />
사용법 3. array를 파라미터 형태로 집어넣고 싶을 때 사용한다.
...
더하기(...어레이);  // 결과 => 60
```
Spread 연산자가 없던 시절엔 apply나 call을 사용했다.
...
apply는 "이 함수를 실행하는데 저기 오브젝트에다 적용해서 실행하시오"라는 의미\
call은 사용법은 같지만, apply처럼 어레이를 집어넣지는 못하고\
1, 2, 3 이렇게 일반 함수처럼 만 집어넣을 수 있다는 차이가 있다.
